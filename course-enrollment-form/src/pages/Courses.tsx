    ...
    return (
        ...
        <div className="categories">
            ...
            >
                ...
                <div className={`category-txt ${selectedCategory === category ? "active" : ""}`}>
                    {category}
                </div>
            </div>
            ))}
        ...
        <div className="lectures">
            ...
            <div className = "course" key={idx}>
                ...
                <div className="date-scope">
                    <span>등록기간: </span>
                    <span>{course.startDate.split("T")[0]}~{course.endDate.split("T")[0]}</span>
                </div>
                <div className="hover-box">
                    <h3>{course.title}</h3>
                    <p style={{lineHeight: 1.2}}>{course.description}</p>
                    <div style={{fontSize: 16}}>남은 자리: {course.maxCapacity - course.currentEnrollment}</div>
                    <div style={{fontSize: 16}}>남은 일: {calDates(course.startDate, course.endDate)}</div>
                    <div style={{fontSize: 16}}>가격: \{course.price.toLocaleString()}</div>
                    <div className="option-group">
                        <button className={`option ${(personalActive && !groupActive) ? "personal-active" : ""}`} onClick={() => {
                        ...
                        <button className={`option ${(groupActive && !personalActive) ? "group-active" : ""}`} onClick={() => {
                    ...
                    <button 
                        ...
                        onClick={() => {
                            personalActive ? navigate("/enrollment-personal", {state: {course}}) : navigate("/enrollment-group")
                        }}>수강신청</button>
                </div>
            </div>
            ))}
